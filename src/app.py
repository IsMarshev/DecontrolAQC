...
binary_classification_model = pipeline('text-classification', model=BertForSequenceClassification.from_pretrained(f"{base_path}models/binary_classification_model"), tokenizer=AutoTokenizer.from_pretrained(base_model_name), device=device)
usefull_classification_model = pipeline('text-classification', model=BertForSequenceClassification.from_pretrained(f"{base_path}models/usefull_classification_model"), tokenizer=AutoTokenizer.from_pretrained(base_model_name), device=device)
useless_classification_model = pipeline('text-classification', model=BertForSequenceClassification.from_pretrained(f"{base_path}models/useless_classification_model"), tokenizer=AutoTokenizer.from_pretrained(base_model_name), device=device)
...
async def upload_file():
    ...
    if uploaded_file and (uploaded_file.filename.endswith('.csv') or uploaded_file.filename.endswith('.xlsx')):
        file_path = os.path.join(f'{base_path}static', uploaded_file.filename)
        print(file_path[-4:])
