# Команда КШАД: Решение кейса "Анализ качества преподавания"
...
4.  docker run -d decontolaqc/decontrolaqc:latest запуск
### ЧЕРЕЗ Py File
3. В коде заменить base_path = './src/' на base_path='' (На пустую строку, чтобы пути начинались не с ./src/)
...
5. Приложение развернется на локальном сервере и подгрузит модели.
6. Сервис позволит выбрать файл для анализа. **КРАЙНЕ РЕКОМЕНДУЕМ** использовать предоставленный нами в репозитории файл (sample.csv) во избежание ошибок в кодировоках, проблемных заголовках, а самле главное наш файл позволит увидеть возможности нашего инструмента. 
